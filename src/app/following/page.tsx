  ...
  useEffect(() => {
    const fetchFollowingStories = async () => {
      ...
      try {
        ...
        const response = await fetch('/api/v1/stories?mode=following', {
          headers: {
        ...
        const data: Story[] = (await response.json()).map((story: any) => ({
      ...
      {stories.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center p-4">
            ...
            <p>Follow some users to see their stories here.</p>
          </div>
        </div>
      ) : (
        <StoryFeed
