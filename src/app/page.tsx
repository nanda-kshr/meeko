...
import { Globe, Users, Plus, Mail, BookOpen } from 'lucide-react';
...
const MeekoApp: React.FC = () => {
  ...
  const renderPageContent = () => {
    ...
    switch (currentPage) {
      ...
      case 'post':
        return <PostPage storyContent={storyContent} setStoryContent={setStoryContent} setCurrentPage={setCurrentPage} onBack={() => setCurrentPage('fyp')} onPost={() => setCurrentPage('fyp')}/>;
      case 'inbox':
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white font-sans">
      ...
      {!showComments && !currentMessage && (
        <div className="bg-gray-900 border-t border-gray-800 py-3 px-2">
          <div className="flex items-center justify-around relative">
            <button onClick={() => setCurrentPage('following')} className={`flex flex-col items-center ${currentPage === 'following' ? 'text-white' : 'text-gray-500'}`}>
              <Users size={24} />
              <span className="text-xs mt-1">Following</span>
            </button>
            <button onClick={() => setCurrentPage('fyp')} className={`flex flex-col items-center ${currentPage === 'fyp' ? 'text-white' : 'text-gray-500'}`}>
              <Globe size={24} />
              <span className="text-xs mt-1">FYP</span>
            </button>
            <div className="absolute left-1/2 -translate-x-1/2 -top-8">
              <button onClick={() => setCurrentPage('post')} className="w-16 h-16 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center shadow-lg border-4 border-gray-900">
                <Plus size={32} className="text-white" />
              </button>
            </div>
            <button onClick={() => setCurrentPage('inbox')} className={`flex flex-col items-center ${currentPage === 'inbox' ? 'text-white' : 'text-gray-500'}`}>
              <Mail size={24} />
              <span className="text-xs mt-1">Inbox</span>
            </button>
            <button onClick={() => setCurrentPage('saved')} className={`flex flex-col items-center ${currentPage === 'saved' ? 'text-white' : 'text-gray-500'}`}>
              <BookOpen size={24} />
